		...
		jQuery('body').on('click', '.wpgmza-paste-color-btn', function(){
			try{
				...
				navigator.clipboard.readText()
				  	.then(function(text) {
				    	colorBtn.parent().find('input[type="color"]').val("#" + text.replace("#").trim());
				  	})
				  	.catch(function(err)  {
...
jQuery(function($) {
	...
	WPGMZA.MarkerGallery = function(marker, context)
	{
		...
		if(marker.gallery.length < 2)
		{
			...
			if(!WPGMZA.settings.disable_lightbox_images && !marker.map.settings.disable_lightbox_images)
			{
...
jQuery(function($) {
	...
	*/
	...
	WPGMZA.ProMap.prototype.onMarkersPlaced = function(event)
	{
		...
		if(self.settings.fit_maps_bounds_to_markers){
	...
	Object.defineProperty(WPGMZA.ProMarker.prototype, "icon", {
		...
		set: function(value) {
			...
			if(value instanceof WPGMZA.MarkerIcon) {
				...
				if (typeof value == "object") {
					value.retina = this.retina && this.retina == '1' ? 1 : 0;
				}
